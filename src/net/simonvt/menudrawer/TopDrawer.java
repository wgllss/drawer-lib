...
public class TopDrawer extends VerticalDrawer {
	...
	public void setDropShadowColor(int color) {
		...
		mDropShadowDrawable = new GradientDrawable(GradientDrawable.Orientation.BOTTOM_TOP, new int[] { color, endColor, });
		invalidate();
	}
	...
	protected boolean onDownAllowDrag(MotionEvent ev) {
		return (!mMenuVisible && mInitialMotionY <= mTouchSize) || (mMenuVisible && mInitialMotionY >= mOffsetPixels);
	}

	@Override
	protected boolean onMoveAllowDrag(MotionEvent ev, float diff) {
		return (!mMenuVisible && mInitialMotionY <= mTouchSize && (diff > 0)) || (mMenuVisible && mInitialMotionY >= mOffsetPixels);
	}
	...
	protected void onUpEvent(MotionEvent ev) {
		...
		if (mIsDragging) {
			...
			animateOffsetTo(mVelocityTracker.getYVelocity() > 0 ? mMenuSize : 0, initialVelocity, true);
